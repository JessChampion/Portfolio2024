<script setup>
import { computed } from "vue";
import { useInkline } from "@inkline/inkline";
import { IButton } from "@inkline/inkline/components";

const { options } = useInkline();
const colorMode = computed(() => options.colorMode);

const toggleColorMode = () => {
  options.colorMode = colorMode.value === "light" ? "dark" : "light";
};
</script>

<template>
  <IButton
    class="switcher"
    :aria-label="`Switch to ${colorMode === 'light' ? 'dark' : 'light'} mode`"
    @click="toggleColorMode"
  >
    <Transition name="celestial-arc">
      <svg
        v-if="colorMode === 'light'"
        key="sun"
        aria-hidden="true"
        role="img"
        class="icon sun"
        width="40px"
        height="40px"
        viewBox="0 0 512 512"
      >
        <path
          fill="currentColor"
          d="M361.5 1.2c5 2.1 8.6 6.6 9.6 11.9L391 121l107.9 19.8c5.3 1 9.8 4.6 11.9 9.6s1.5 10.7-1.6 15.2L446.9 256l62.3 90.3c3.1 4.5 3.7 10.2 1.6 15.2s-6.6 8.6-11.9 9.6L391 391l-19.9 107.9c-1 5.3-4.6 9.8-9.6 11.9s-10.7 1.5-15.2-1.6L256 446.9l-90.3 62.3c-4.5 3.1-10.2 3.7-15.2 1.6s-8.6-6.6-9.6-11.9L121 391L13.1 371.1c-5.3-1-9.8-4.6-11.9-9.6s-1.5-10.7 1.6-15.2L65.1 256L2.8 165.7c-3.1-4.5-3.7-10.2-1.6-15.2s6.6-8.6 11.9-9.6L121 121l19.9-107.9c1-5.3 4.6-9.8 9.6-11.9s10.7-1.5 15.2 1.6L256 65.1l90.3-62.3c4.5-3.1 10.2-3.7 15.2-1.6M160 256a96 96 0 1 1 192 0a96 96 0 1 1-192 0m224 0a128 128 0 1 0-256 0a128 128 0 1 0 256 0"
        ></path>
      </svg>
      <svg
        v-else
        key="moon"
        aria-hidden="true"
        role="img"
        class="icon moon"
        width="40px"
        height="40px"
        viewBox="0 0 384 512"
      >
        <path
          fill="currentColor"
          d="M223.5 32C100 32 0 132.3 0 256s100 224 223.5 224c60.6 0 115.5-24.2 155.8-63.4c5-4.9 6.3-12.5 3.1-18.7s-10.1-9.7-17-8.5c-9.8 1.7-19.8 2.6-30.1 2.6c-96.9 0-175.5-78.8-175.5-176c0-65.8 36-123.1 89.3-153.3c6.1-3.5 9.2-10.5 7.7-17.3s-7.3-11.9-14.3-12.5c-6.3-.5-12.6-.8-19-.8z"
        ></path>
      </svg>
    </Transition>
  </IButton>
</template>

<style lang="scss">
@use "../inkline-variables/mixins.scss" as *;

.switcher.button {
  background: var(--body--background);
  border: none;
  border-radius: 0 0 var(--border-radius-rounded) var(--border-radius-rounded);
  box-shadow: none;
  padding: var(--gap-15) var(--gap-2) var(--gap-125);
  overflow: hidden;
  transition: var(--transition-background-color), var(--transition-color);
  position: relative;
  width: 7.5rem;
  height: 6rem;

  @include breakpoint-sm-down {
    padding: var(--gap) var(--gap) var(--gap);
  }

  .icon {
    position: absolute;
    left: calc(50% - 20px);
    top: calc(50% - 20px);
    display: block;
    transform-origin: 50% 210% !important;
  }

  .celestial-arc-enter-active {
    animation: celestialSunrise 0.5s cubic-bezier(0.25, 1, 0.5, 1) 0.4s both !important;
  }

  .celestial-arc-leave-active {
    animation: celestialSunset 0.4s cubic-bezier(0.4, 0, 1, 1) forwards !important;
  }
}

@keyframes celestialSunrise {
  0% {
    opacity: 0;
    transform: rotate(-130deg) scale(0.65);
  }
  30% {
    opacity: 1;
  }
  100% {
    opacity: 1;
    transform: rotate(0deg) scale(1);
  }
}

@keyframes celestialSunset {
  0% {
    opacity: 1;
    transform: rotate(0deg) scale(1);
  }
  100% {
    opacity: 0;
    transform: rotate(130deg) scale(0.65);
  }
}
</style>
