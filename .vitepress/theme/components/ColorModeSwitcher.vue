...
export default defineComponent({
  ...
  setup() {
    ...
    const colorMode =
      (computed < "light") | ("dark" > (() => options.colorMode));
    console.log({ colorMode, options });
...
<template>
  <IButton
    class="switcher"
    aria-label="`Switch to ${colorMode: colorMode==='light' ? 'dark' : 'light'} mode`"
...
.switcher.button {
  ...
  .icon {
    ...
    transition:
      color var(--transition-timing-function) var(--transition-duration),
      hover var(--transition-timing-function) var(--transition-duration);
  }
